	...
	server.registerPrompt(
		...
		() => {
			const text = [
				...
				"",
				"Compact alternatives (lower token cost, same data):",
				`- ${URIS.runtimeStatsToon} — TOON-encoded stats table (compact tabular format).`,
				`- ${URIS.runtimeEventsAggToon} — TOON-encoded aggregated events table.`,
				"",
				"Static structure:",
				`- ${URIS.graph} — project-wide Nanostores dependency graph (files, stores, relations).`,
				"",
