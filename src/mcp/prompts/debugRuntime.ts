				...
				messages: [
					{
						...
						content: {
							...
							text: [
								...
								`- ${URIS.runtimeStoreTemplate.replace("{key}", store_name)} - **Enhanced store profile** with both static (file, type, relations) and runtime (events, metrics) data automatically merged`,
								"",
								"### Additional Detail (if needed)",
								`- ${URIS.storeById(store_name)} - Pure static analysis (AST graph, subscribers, relations)`,
								...
								"## Analysis Framework",
								"",
								"The runtime store profile includes merged static + runtime data:",
								"- `id`, `kind`, `file` - from static AST analysis (if available)",
								"- `stats`, `recentEvents`, activity metrics - from runtime logger",
								"- This allows you to correlate code structure with actual behavior",
								"",
